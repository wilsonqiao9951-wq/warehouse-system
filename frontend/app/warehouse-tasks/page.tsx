...
export default function WarehouseTasksPage() {
  ...
  return <ManagerShell title="Warehouse tasks" subtitle="把工程师消耗转成可执行的补货和调拨任务。" metrics={[{ label: "Open alerts", value: notifications.length }, { label: "Requests", value: requests.length }, { label: "Workflow", value: "Live" }]}>
    ...
    <section className="card"><h3>Replenishment requests</h3>{requests.length === 0 ? <div className="empty-state">Requests created from alerts will appear here.</div> : <div className="table-wrap"><table><thead><tr><th>Part</th><th>Destination</th><th>Source</th><th>Qty</th><th>Status</th></tr></thead><tbody>{requests.map((item) => <tr key={item.id}><td>#{item.part_id}</td><td>#{item.destination_warehouse_id}</td><td>{item.source_warehouse_id ? `#${item.source_warehouse_id}` : "To assign"}</td><td>{item.quantity}</td><td>{item.status}</td></tr>)}</tbody></table></div>}</section>
  </ManagerShell>;
}
