...
以下为 **逐条勾选** 的推荐顺序。每条记录：设备型号、系统版本、浏览器、已认证账号角色、设备注册状态、通过/失败、截图或安全日志引用。
...
> 当前试点使用正式邮箱/密码会话；角色由服务器决定。管理员需要 MFA，工程师写入需要注册手机与当前工单 claim。

3. **Technician 模拟**
   - 用工程师自己的邮箱/密码登录，在 Profile 确认手机已注册。
   - 确认底栏：Today / Map / My Jobs / My Van / Profile 可切换，接口请求正常。
4. **Manager 模拟**
   - 用经理自己的邮箱/密码登录，确认服务器返回经理角色。
...
3. 手机浏览器打开前端 URL → 使用本人账号登录并确认设备状态 → 按第三节顺序执行。
